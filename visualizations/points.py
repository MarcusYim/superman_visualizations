...
def plot_points(fig: Figure, points: np.ndarray, i_param: Optional[np.ndarray] = None, size: int = 10):
    """
    ...
    if dim not in (2, 3):
        raise ValueError(f"Points dimension {points.shape[1]}, expected dimension 2 or 3!")
    ...
    if dim == 2:
        ...
        sc = ax.scatter(points[:, 0], points[:, 1], points[:, 2], c=i_param, s=size)
